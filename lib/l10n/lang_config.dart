...
class LangConfig extends ChangeNotifier {
  ...
  void linkSetState(VoidCallback fn) => addListener(fn);

  /// 关联 setState 实现自动刷新, shortcut for `instance.linkSetState(fn)`
  /// 用于在语言切换时自动刷新页面, 建议在页面的 initState 中调用
  static void link(VoidCallback fn) => instance.linkSetState(fn);

  /// 取消关联 setState
  /// 用于在 dispose 时取消关联
  /// 建议使用 `LangConfig.unlink(fn)` 取消关联
  void unlinkSetState(VoidCallback fn) => removeListener(fn);

  /// 取消关联 setState, shortcut for `instance.unlinkSetState(fn)`
  static void unlink(VoidCallback fn) => instance.unlinkSetState(fn);
